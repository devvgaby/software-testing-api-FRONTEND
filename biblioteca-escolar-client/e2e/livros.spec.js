...
test.describe('Gerenciamento de Livros (E2E)', () => {
  ...
  test('deve permitir adicionar um novo livro e encontrá-lo na lista', async ({ page }) => {
    ...
    const responsePromise = page.waitForResponse(
      resp => resp.url().includes('/livros') && resp.request().method() === 'POST' && resp.status() >= 200 && resp.status() < 300
    );
    ...
    await page.fill('input[placeholder = "Buscar por ID..."]', String(novoLivro.id));
  ...
  test("deve permitir editar livro", async ({ page}) => {
    ...
    await expect(page.locator('.modal')).not.toBeVisible();
    await expect(page.locator('.list-card', { hasText: tituloOriginal + " - Editado"}))
      .toBeVisible();  
  });
});