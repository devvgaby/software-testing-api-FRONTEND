...
test.describe("Gerenciamento de Usuários (E2E)", () => {
  ...
  test("deve navegar até a tela de usuários e listar usuários", async ({
    page,
  }) => {
  ...
  test("deve permitir adicionar um novo usuário e encontrá-lo na lista", async ({
    page,
  }) => {
  ...
  test("deve permitir editar um usuário", async ({ page }) => {
    ...
    const nomeOriginal = await primeiroUsuario
      .locator(".list-card__title")
      .innerText();
    ...
    await expect(
      page.locator(".list-card", { hasText: `${nomeOriginal} - Editado` }),
    ).toBeVisible();
  });
});
